...
void ObjReader::ReadObjSize(std::string file) {
	...
	v = new MyVec3[vSize + 1];
	vt = new std::pair<double, double>[vtSize + 1];
	if (vnSize == 0)
		vn = new MyVec3[vSize + 1];
	else
		vn = new MyVec3[vnSize + 1];
...
void ObjReader::CalnVn() {
	if (vnSize > 0) {
		for (int i = 1; i <= vnSize; i++) {
			vn[i] = vn[i].Rotate(MyVec3(1, 0, 0), polyhedron->GetAngles().GetCoord(0));
			vn[i] = vn[i].Rotate(MyVec3(0, 1, 0), polyhedron->GetAngles().GetCoord(1));
			vn[i] = vn[i].Rotate(MyVec3(0, 0, 1), polyhedron->GetAngles().GetCoord(2));
		}
	} else {
		cout << "1996 - 04 - 05" << endl;
		exit(43);
	}
}
...
void ObjReader::ReadObj(std::string file) {
	...
	while (getline(fin, order, '\n')) {
		...
		if (var == "f") {
			...
			//tri->SetSamp   le(polyhedron->GetSample());
			if (matID != -1)
	...
	TriangleNode* root = polyhedron->GetTree()->GetRoot(); //根节点
	root->size = fCnt; //面的数目
	root->tris = new Triangle*[root->size]; //三角面片数
